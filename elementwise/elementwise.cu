...
__global__ void elementwise_add_f16x8_kernel(half* a, half* b, half* c, int N)
{
	...
	reg_c_0.x = _hadd(reg_a_0.x, reg_b_0.x);
	reg_c_1.x = _hadd(reg_a_1.x, reg_b_1.x);
	reg_c_2.x = _hadd(reg_a_2.x, reg_b_2.x);
	reg_c_3.x = _hadd(reg_a_3.x, reg_b_3.x);
	reg_c_0.y = _hadd(reg_a_0.y, reg_b_0.y);
	reg_c_1.y = _hadd(reg_a_1.y, reg_b_1.y);
	reg_c_2.y = _hadd(reg_a_2.y, reg_b_2.y);
	reg_c_3.y = _hadd(reg_a_3.y, reg_b_3.y);
	if ((idx + 0) < N) {
