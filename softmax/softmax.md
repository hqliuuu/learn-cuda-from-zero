...
发现一个问题，我的矩阵是4096*256。横坐标是hidden_size，纵坐标token。然后我就发现我不能随意的修改block_size，要和hidden_size契合。
因为如果这样的话，那我的softmax的分母算的就不是全局的和了。

上面的测试结果是因为我全部bench_mark的时候都调用到一个算子去了。

但是这个优化的方法是没问题的。这个在block内加的方法是会快一些。
```
----------------------------------------------------------------------------------------------------
                                             S=4096, H=256
----------------------------------------------------------------------------------------------------
       out_softmax_naive: ['0.00180731  ', '0.00099762  ', '0.0025904   '], time:0.54604053ms
     out_softmax_naive_1: ['0.00510912  ', '0.00365836  ', '0.00705731  '], time:0.52515030ms
out_softmax_block_reduce: ['0.00180731  ', '0.00099762  ', '0.0025904   '], time:0.07029057ms
       out_softmax_torch: ['0.00180731  ', '0.00099762  ', '0.0025904   '], time:0.05005360ms
